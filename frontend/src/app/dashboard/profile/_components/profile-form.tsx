...
export function ProfileForm() {
   ...
   * Derived, not synced.
   *
   * `useState(profile.x)` cannot work against a query — the first render has
   * no profile and an initial value is never read again — and an effect that
   * calls `setState` cascades a render every time the query settles. So the
   * draft holds only what the person has TYPED, and the rest reads through to
   * the server's answer.
   */
  const [draft, setDraft] = React.useState<Partial<ProfileDraft>>({})

  const form: ProfileDraft | null = profile
    ? {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    update.mutate(form, {
      onSuccess: () => {
        // Cleared so the form reads through to the server's answer again.
        setDraft({})
  ...
  /*
   * `email` is NOT here, and that is the API's rule rather than an omission.
   *
   * `profileUpdateSchema` refuses it: an address is what a booking
   * confirmation and a password reset are sent to, so changing it is a
   * verification flow, not a text field. The form used to offer it, accept the
   * edit, and write it nowhere.
   */
  const fields = [
    ...
    <Card>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label>Email</Label>
            <Input value={profile.email} readOnly disabled autoComplete="email" />
            <p className="text-muted-foreground text-xs">
              Your bookings and password resets are sent here. Contact support to
              change it.
            </p>
          </div>
