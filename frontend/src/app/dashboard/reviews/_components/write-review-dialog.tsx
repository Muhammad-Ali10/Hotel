...
import type { ReviewCategories } from "@/types"
import { REVIEW_CATEGORIES } from "@/types"
import { cn } from "@/lib/utils"
import type { ReviewableBooking } from "@/lib/api/endpoints"
...
export function WriteReviewDialog({
  ...
  function submit() {
    ...
    /*
     * Only the booking id and the words.
     *
     * The property, the room name, the author and the country all come off the
     * BOOKING on the server — sending them from here would let a client claim
     * a review was about a different hotel, and would be a second copy of
     * facts the booking already holds.
     */
    create.mutate(
      {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Review {booking.propertyName}</DialogTitle>
          <DialogDescription>
