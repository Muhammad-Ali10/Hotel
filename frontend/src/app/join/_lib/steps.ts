...
export const steps: WizardStep[] = [
  ...
  { slug: "description", path: "description", step: 7, chrome: "wizard" },
...
/**
 * Where a returning applicant belongs.
 *
 * `currentStep` is the Figma number the API stores with every save, so it maps
 * back through the registry rather than through a second table of routes. An
 * unknown number — a draft saved before a screen was renumbered — falls back to
 * the first wizard step instead of throwing them back to the account screens
 * they have already finished.
 */
export function resumeHref(currentStep: number | undefined): string {
  const match = steps.find((s) => s.step === currentStep && s.chrome === "wizard")
  return href(match?.path ?? "property-type")
}
