...
import type { WizardUnit } from "../../_lib/types"

const unitTypes = ["Room", "Apartment", "Studio", "Suite", "Bungalow", "Villa"].map(
  (v) => ({ value: v, label: v }),
)

const bedRows: { key: keyof WizardUnit["beds"]; label: string; width: string }[] = [
...
export default function RoomDetailsPage() {
  ...
  const [unit, setUnit] = React.useState<WizardUnit>(data.draftUnit)

  const patch = (p: Partial<WizardUnit>) => setUnit((u) => ({ ...u, ...p }))
  const setBed = (key: keyof WizardUnit["beds"], value: number) =>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Counter
            ...
            suffix="guests"
          />
          {/* The "exclude infants" checkbox that sat here set a local boolean
              and nothing else. There is no infant concept in this system —
              occupancy is adults and children — so ticking it changed neither
              what a guest could book nor what the room was priced at. */}
        </div>
      ...
      <StepNav slug="unit-details" onContinue={() => save({ draftUnit: unit })} />
    </WizardShell>
  )
}
