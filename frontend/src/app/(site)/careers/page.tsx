...
const benefits: { icon: LucideIcon; title: string; description: string }[] = [
  ...
  },
  {
    ...
    title: "Remote first",
    description:
      "Work from where you are effective. Tell us where you are and what hours suit you, and we will say honestly whether it fits the team you would join.",
  },
  {
    icon: Clock3,
 ...
 * Open roles.
 *
 * Six were listed here — a senior frontend engineer, a payments backend
 * engineer, a product designer, a partnerships manager in the GCC, and two more
 * — each with a team, a location and a contract type, under a heading that
 * counted them as "positions open" and a badge reading "Updated weekly".
 *
 * None of them was a vacancy. A person reading that page writes a covering
 * letter, adjusts a CV and applies for a job nobody is hiring for; the cost of
 * an invented job advert is paid entirely by the applicant.
 *
 * Empty until there are real ones. The section below already renders the empty
 * case, so adding a genuine opening here is the only change needed.
 */
const roles: {
  title: string
  team: string
      ...
      <section className="border-b bg-muted/30">
        <div className="mx-auto max-w-7xl px-4 py-16 text-center sm:px-6 lg:px-8 lg:py-20">
          ...
          <p className="text-muted-foreground mx-auto mt-4 max-w-2xl text-lg text-pretty">
            We are a small, senior team building a marketplace that people
            trust with the trip they have been saving for. If that sounds like
            your kind of problem, we would like to hear from you.
          </p>
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <p className="text-muted-foreground mt-2">
                {roles.length > 0
                  ? `${roles.length} ${roles.length === 1 ? "position" : "positions"} open right now.`
                  : "Nothing open at the moment — but we read everything that arrives."}
              </p>
            </div>
          </div>
