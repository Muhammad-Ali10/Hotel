...
export const bookingStatusStyle: Record<BookingStatus, string> = {
  ...
  // A no-show is not a cancellation: nobody chose it, and the property is
  // usually owed something for it (rule #47). Its own colour, deliberately.
  no_show: "border-transparent bg-violet-500/10 text-violet-700 dark:text-violet-400",
...
/**
 * How an extra is charged, in words.
 *
 * The values are the API's (`per_person_per_night`), and they are not
 * something to show a guest. Rendering the raw enum is how "per_person" ends
 * up on a checkout page.
 */
export const valueAddUnitLabel: Record<ValueAddUnit, string> = {
  per_stay: "per stay",
  per_night: "per night",
