...
export function valueAddPrice(
  ...
  switch (valueAdd.unit) {
    ...
    /*
     * The unit rule #10 exists for.
     *
     * "Daily breakfast" priced `per_person` was charged $32 ONCE for a
     * three-night stay for two. Its absence here was invisible until the
     * frontend adopted the API's vocabulary and the compiler pointed at this
     * switch.
     */
    case "per_person_per_night":
      return valueAdd.price * guests * nights
  }
}
 ...
 */
/**
 * @deprecated The server prices stays. This does not agree with it.
 *
 * A second implementation of `POST /properties/:slug/quote`, and since the
 * catalogue went live it is a WRONG one:
 *
 *  - per-night rates come from `hotel.availability.rateOverrides`, which the
 *    API adapter deliberately leaves empty — so every night comes out at the
 *    cheapest rate plan's flat base price
 *  - `hotel.discount` is never populated, so a promotion is never applied
 *  - it still computes a tax layer, which rule #11 removed from the product
 *
 * `ReserveCard` no longer calls it. Five callers remain — checkout, the search
 * cards, modify-booking and the extranet's new-reservation dialog — and each
 * goes as its screen is wired to the API. **Do not add a sixth.**
 */
...
/**
 * The published average, and the category averages under it.
 *
 * Takes the three fields it actually reads rather than a whole `Review`, so it
 * works against the API's shape as well as the fixture's — the two agree on
 * `rating`, `categories` and `status` and on almost nothing else.
 */
...
/**
 * Whoever the stay is FOR.
 *
 * Takes the guest block rather than a whole `Booking`, so it works against the
 * API's shape as well as the fixture's — the two disagree on nearly every
 * other field, and this function only ever needed two of them.
 */
