...
/**
 * The API's vocabulary, verbatim.
 *
 * Capitalised here once — `"Hotel"` — while the API has always had `"hotel"`.
 * The same divergence `ValueAddUnit` had, and the same fix: the display label
 * belongs in `lib/labels`, not in the value that gets sent over the wire.
 */
 ...
 * The API's vocabulary, verbatim.
 *
 * This used to be three values with spaces — `"per_stay"` — while the API has
 * always had four with underscores. Two of the same thing, in the shape
 * `AGENTS.md` warns about, and the missing fourth was
 * `per_person_per_night`: the unit rule #10 exists FOR, after "Daily
 * breakfast" was priced per person and charged $32 once for a three-night
 * stay.
 */
export type ValueAddUnit =
  | "per_stay"
  | "per_night"
  | "per_person"
...
/**
 * The API's own list, and nothing else.
 *
 * It used to carry `checked_out`, which the API has never had, and to omit
 * `no_show`, which it does. A no-show therefore rendered as a blank badge and
 * a `checked_out` case that could never arrive was being styled.
 *
 * The sixth vocabulary divergence in this codebase, fixed the same way as the
 * others: adopt the API's words.
 */
...
/** The API's words. Display names are produced by `bookingSourceLabel`. */
